...
//     }
    ...
    // //If gene is to be mutated, run the chance of mutation past each chromosome
    // vector<double> childMutationChance(numOfChildren, 0);
    // for (int c = 0; c < childMutationChance.size(); c++) {  // Populate random chance of mutation for each bit
    //     childMutationChance[c] = static_cast<double>(rand()) / RAND_MAX;
    // }
    ...
    //     if (childMutationChance[c] > (1 - mutationRate)) {
    //         for (int i = 0; i < pop[0].size(); i++) {
    //             double mutateChance = static_cast<double>(rand()) / RAND_MAX;
    //             if (mutateChance > 1 - mutationRate) {
    //                 children[c][i] = 1 - children[c][i];
    //             }
    //         }
    //     }
    // }

    // //If the gene is to be mutated, flip randome chromosome in gene
    // vector<double> childMutationChance(numOfChildren, 0);
    // for (int c = 0; c < childMutationChance.size(); c++) {  // Populate random chance of mutation for each bit
    //     childMutationChance[c] = static_cast<double>(rand()) / RAND_MAX;
    // }
    // for (int c = 0; c < childMutationChance.size(); c++) {  // MUTATION IMPLEMENTATION - CHANCE OF MUTATION APPLIED TO EACH CHROMOSOME
    //     if (childMutationChance[c] > (1 - mutationRate)) {
    //         int mutatedIndex = rand() % pop[0].size();
    //         children[c][mutatedIndex] = 1 - children[c][mutatedIndex];
    //     }
    // }
...
/* Begin algorithm logic */
    while (averageFitnessScore < 1.0 && generationNumber < safetyLimit) {// && (highestFitnessScore < 1.0 || averageFitnessScore < .80 || percentOfIdenticalGenomes < 90)) {
    ...
    if (averageFitnessScore >= 1.0) {
        ...
        cout <<endl;
        printPopulation(population);
    } /*else if (highestFitnessScore >= 1.0 && averageFitnessScore > .80 && percentOfIdenticalGenomes >= 90) {
